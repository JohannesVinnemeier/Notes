...
Let us begin by recalling the definition of these topolgies. First we define the Whitney topology on the space $C^k(M,N)$ of $C^k$ maps from $M$ to $N$. We will be using the definition from \cite{hirsch2012differential} using local charts. For the discussion of an equivalent definition see section \ref{whitney_top:sec:equivalent_def}. Suppose we are given the following data
\begin{itemize}
...
\end{itemize} 
Then if $f \in C^k(M,N)$ with $f(K_\alpha) \subseteq V_\alpha$ for each $\alpha \in J$ we define a basic open neighbourhood
\[
\mathcal{N}^k(f,\Phi,\Psi,K,\epsilon)
\]
as the set of functions $g \in C^k(M,N)$, such that for all $\alpha \in J$, $g(K_\alpha) \subseteq V_\alpha$ and the coordinate representations $f_\alpha := \psi_\alpha^{-1} \circ f\circ\phi_\alpha$ and $g_\alpha := \psi_\alpha^{-1} \circ g\circ\phi_\alpha$ defined on some open neighbourhood of $\phi_\alpha^{-1}(K_\alpha)$ satisfy 
\[
...
\]
The sum is taken over multiindices $\beta \in \N_0^n$ (where $n = \mathrm{dim}(M)$). The collection of all such basic open neighbourhoods form the basis for a topology on $C^k(M,N)$ (it is easy to see that the intersection of two such neighbourhoods is again of the same form). The topology on $C^\infty(M,N)$ is then simply defined as the topology induced by all inclusion maps $C^\infty(M,N) \to C^k(M,N)$. 
...
\footnotetext{The reason for this is to avoid discussions about the necessary regularity conditions needed for Sard's Theorem.}
...
Increasing the dimension of $N$ by one we even obtain the following.
...
Finally we have a statement about transversality. 
...
Together all of these results comprise a very efficient machinery for tackling problems in differential topology. As an example we proof the following, which - up to the precise restrictions on the dimension - is a generalisation of Theorem \ref{constr_diffeo:thm:equivalence_of_embeddings}.
...
In this section we will proof Theorems \ref{whitney_top:thm:approx_immersions}, \ref{whitney_top:thm:approx_one_to_one_immersions} \ref{whitney_top:thm:approx_transversality} as well as Proposition \ref{whitney_top:prop:homotopy_construction}. The proofs of Theorem \ref{whitney_top:thm:approx_immersions} and \ref{whitney_top:thm:approx_one_to_one_immersions} are generalisations of the proofs of Theorem 2.5 and Lemma 2.5 in \cite{adachi1993embeddings} respectively. 

In the following we will necessarily need to compute some $C^k$-norms. The following technical Lemma takes care of all of such computations which we need.

\begin{lemma}
    \label{whitney_top:lem:technical_lemma_about_norms}
    Suppose we are given open sets $U, U^\prime \subseteq \H^n, W, W^\prime \subseteq \H^m$, compact subsets $K \subseteq U, L \subseteq W$, a smooth function $\lambda \colon U \to \R$, diffeomorphisms $\phi \colon U^\prime \to U$ and $\psi \colon W \to W^\prime$ as well as constants $\epsilon > 0$ and $C > 0$. Then there exists $\delta >0$ with the following property. Let $r,s, \theta \colon U_K \to W$ be smooth functions defined on some neighbourhood $U_K \subseteq U$ of $K$, which can depend on the functions $r,s,\theta$. Suppose that $s$ satisfies $s(K) \subseteq L$ and $\norm{s}_{C^k(K)} \leq C$. Then if
    \[
...
\begin{proof}
    ...
    \norm{S_\beta^\alpha - R_\beta^\alpha}_{C^0(K)}
    \]
    can be made arbitrarily small for sufficiently small $\delta$. To see this note that $(\partial^\gamma r_j)_{\abs{\gamma} \leq l, j = 1,\ldots,m}$ restrcited to $K$ only takes values in the compact set $I^n$ defined above. The same holds for $(\partial^\gamma s_j)_{\abs{\gamma} \leq l, j = 1,\ldots,m}$. Hence the statement follows from the fact that $P_\beta^\alpha$ is uniformly continuous on $I^n$. This shows that $\delta$ can be chosen such that (\ref{whitney_top:eq:ck_norm_lemma_part1}) holds. 
    ...
    \]
    for any multiindex $\alpha \in \N_0^n$ of order $\abs{\alpha} = l \leq k$, this time for different polynomials $T_\beta^\alpha$. Again we abbreviate $\Phi_\beta^\alpha := T_\beta^\alpha((\partial^\gamma \phi_j)_{\abs{\gamma} \leq l, j = 1,\ldots,n})$. We now have to choose $\delta >0$, such that  
    \[
    ...
    \[
    \partial^\alpha(\lambda \theta) = \sum_{\abs{\beta} + \abs{\gamma} = l} \partial^\beta \lambda \, \partial^\gamma \theta
    \]
    for any multiindex $\alpha \in \N_0^n$ of order $\abs{\alpha} = l \leq k$. Since 
    \[
    \norm{\partial^\beta \lambda \, \partial^\gamma \theta}_{C^0(K)} \leq \norm{\partial^\beta \lambda}_{C^0(K)} \norm{\partial^\gamma \theta}_{C^0(K)} < \delta \norm{\partial^\beta \lambda}_{C^0(K)}
    \]
    and $\norm{\partial^\beta \lambda}_{C^0(K)}$ is just some constant not depending on $\theta$ the statement follows.  
\end{proof}


We now begin with the proof of Theorem \ref{whitney_top:thm:approx_immersions} for which will need the following Lemma, which is essentially a local version of the theorem we aim to proof. 
...
\begin{proof}
    First we argue that we may assume $\partial U = \emptyset$. If this is not the case we can use a partition of unity to extend $f$ to a smooth function $f^\prime \colon U^\prime \to \R^p$ defined on an open subset $U^\prime \subseteq \R^n$ containing $U$. Then apply the Lemma to $f^\prime$ and restrict to $U$ again. So assume $\partial U = \emptyset$. Define 
    \[
...
\begin{proof}[Proof of Theorem \ref{whitney_top:thm:approx_immersions}]
    ...
    Moreover we can apply Lemma \ref{whitney_top:lem:approx_immersions_lemma} to choose $A_n$, such that $f_n$ is an immersion on an open neighbourhood of $B_n$ on which $\lambda_n = 1$, which exists by (\ref{whitney_top:eq:immersion_cover_property1}). In particular it is an immersion on $B_n$. We can furthermore guarantee that $f_n$ remains an immersion on $C_{n-1}$ by choosing $A_n$ sufficiently small. To see this note that $C_{n-1}$ is closed by local finiteness of $(O_i)_{i \in \Z}$, and so in particular $D_n \cap C_{n-1}$ is compact. The claim then follows essentially from the fact that the set of $p \times n$ matrices of full rank is open in $\R^{p \times n}$. Thus \ref{whitney_top:eq:immersion_property_1}) is also established.
    ...
    \[
    \norm{f_n^\alpha - f_{n-1}^\alpha}_{C^k(\phi_\alpha^{-1}(K_\alpha))} = \sum_{\abs{\beta} \leq k} \max_{x \in \phi_\alpha^{-1}(K_\alpha \cap D_n)} \norm{\partial^\beta(\tau_n^\alpha \circ (\hat{f}_{n-1}^\alpha + \hat{A}_n^\alpha))(x) - \partial^\beta(\tau_n^\alpha \circ \hat{f}_{n-1}^\alpha)(x)}.
    \]
    Note that we can make $\hat{A}_n^\alpha$ and all of its partial derivatives arbitrarily small by choosing $A_n$ sufficiently small. Thus we can apply Lemma \ref{whitney_top:lem:technical_lemma_about_norms} (\ref{whitney_top:eq:ck_norm_lemma_part1}) to 
    \begin{itemize}
    ...
    \]
    This is a well defined smooth map, since by local finiteness of $(O_i)_{i \in \Z}$ we get that an arbitrary point $x \in M$ has a neighbourhood $U(x)$ which intersects only finitely many $D_i$. Thus for sufficiently large $n > 0$ we conclude from \ref{whitney_top:eq:immersion_property_2}) that
    \[
    ...
    \]
    is again contained in the domain of $w_n$. Moreover this added term $\lambda_n(x) \delta_n e_1$ does not change the fact that $f_n$ is an immersion on $B_n$ for almost all $A_n$, since it is constant a neighbourhood of $B_n$. The rest of the proof then goes through unchanged if in addition to $A_n$ we also choose $\delta_n$ sufficiently small in each step.
\end{proof}
...
\begin{proof}[Proof of Theorem \ref{whitney_top:thm:approx_one_to_one_immersions}]
    ...
    \begin{enumerate}[(i)]
        ...
        \item \label{whitney_top:eq:one_to_one_immersion_cover_property5}$f|_{B_i}$ is injective.
    \end{enumerate}
    Again the existence of this data follows from paracompactness of $M$. For (\ref{whitney_top:eq:one_to_one_immersion_cover_property5}) recall that immersions are locally embeddings. Now define a sequence $f_n \colon M \to N$, $n \geq 0$, with $f_0 := f$, satisfying the following properties.
    \begin{enumerate}[1)]
        ...
        \]
    \end{enumerate}
    Here properties \ref{whitney_top:eq:one_to_one_immersion_property_2})- \ref{whitney_top:eq:one_to_one_immersion_property_4}) are the same as in the proof of Theorem \ref{whitney_top:thm:approx_immersions}. Again it is clear that $f_0 = f$ satisfies these properties. At first we want to assume taht both $M$ and $N$ have empty boundary. We define $f_n$ using $f_{n-1}$ by
    \[
    f_n(x) := \begin{dcases}
    ...
    \]
    Since $\mathrm{dim}(G_n) = 2\,\mathrm{dim}(M) < p$ the image of $\gamma_n$ is a set of zero measure. So (\ref{whitney_top:eq:claim_one_to_one_immersions}) holds for all $b_n$ not contained in this set. Let us now choose such $b_n$ to define $f_n$. Using (\ref{whitney_top:eq:claim_one_to_one_immersions}) it is now easy to show that $f_n$ is injective on $C_n$. To this end assume there exist $x,y \in C_n$ with $f_n(x) = f_n(y)$ and $x \neq y$. Applying (\ref{whitney_top:eq:claim_one_to_one_immersions}) inductively shows 
    \[
...
\begin{proof}
    ...
    \]
    Since $d_{(x,b)} \pi$ is just the restriction of the projection $T_xM \oplus T_b B \to T_b B$ it follows that $z_1 = z_2$. So by linearity we can write (\ref{whitney_top:eq:approx_transversality_lemma2})
    \[
...
\begin{proof}[Proof of Theorem \ref{whitney_top:thm:approx_transversality}]
    ...
    \end{enumerate}
    Again the existence of this data follows from paracompactness of $M$. Now define a sequence $f_n \colon M \to N$, $n \geq 0$, with $f_0 := f$, satisfying the following properties.
    \begin{enumerate}[1)]
...
Finally we proof Proposition \ref{whitney_top:prop:homotopy_construction}. In the proof we will repeatedly make use of the following technical lemma. The precise formulas in this lemma may look strange at first but it will become clear in the proof of Proposition \ref{whitney_top:prop:homotopy_construction} where they comes from.

\begin{lemma}
    \label{whitney_top:lem:lemma_for_homotopy_prop}
    Suppose we are given open sets $U, U^\prime \subseteq \H^n, W, W^\prime \subseteq \H^m$, $V \subseteq W$ compact sets $K \subseteq U, L \subseteq V$, a smooth function $\lambda \colon U^\prime \to [0,1]$, diffeomorphisms $\phi \colon U^\prime \to U$ and $\psi \colon W \to W^\prime$ as well as constants $\epsilon > 0$ and $C > 0$. Then there exists $\delta >0$ with the following property. Let $p,q \colon U_K \to W$ be smooth functions defined on some neighbourhood $U_K \subseteq U$ of $K$, which can depend on the functions $p,q$. Suppose that $q$ satisfies $q(K) \subseteq L$ and $\norm{q}_{C^k(K)} \leq C$. Then if
    \[
    \norm{p-q}_{C^k(K)} < \delta
    \]
    we have 
    \begin{enumerate}[(i)]
        \item \label{whitney_top:eq:lemma_for_homotopy_prop_part1} $((1-\lambda)p + \lambda q)(K) \subseteq V$ and
    \[
        \norm{\psi((1-\lambda) (p \circ \phi) + \lambda (q \circ \phi)) - \psi \circ p \circ \phi}_{C^k(\phi^{-1}(K))} < \epsilon.
    \]
    \item \label{whitney_top:eq:lemma_for_homotopy_prop_part2} $((1-\lambda)(\psi \circ p) + \lambda (\psi \circ q))(K) \subseteq \psi(V)$ and 
    \[
    \norm{\psi^{-1}((1-\lambda) (\psi \circ p) + \lambda (\psi \circ q)) - q}_{C^k(K)} < \epsilon.
    \]
    \end{enumerate}
\end{lemma}

\begin{proof}
    We first choose $\delta$, such that $((1-\lambda)p + \lambda q)(K) \subseteq V$ holds. By compactness of $L \subseteq V$ there exists some radius $r > 0$ depending only on $L$ and $V$ with the property that any ball of radius $r$ around a point in $L$ is contained in $V$. So choosing $\delta < r$ it follows from $q(K) \subseteq L$ and $0 \leq \lambda \leq 1$, that $(1-\lambda)p(x) + \lambda q(x) \in V$ for any point $x \in K$. In fact the same argument shows that if we fix some compact neighbourhood $\tilde{L} \subseteq W$ of $L$ we can choose $\delta$ sufficiently small, such that $p(K) \subseteq \tilde{L}$ is automatically satisfies. We will make use of this fact later in the proof. 
    ...
    \[
    \norm{\lambda (p \circ \phi - q \circ \phi)}_{C^k(\phi^{-1}(K))}
    \]
    ...
    \begin{align*}
        r &= p \circ \phi + \lambda (p \circ \phi - q \circ \phi) \\
        s &= p \circ \phi
    \end{align*}
    to conclude that 
    \[
    \norm{\psi(p \circ \phi + \lambda (p \circ \phi - q \circ \phi)) - \psi \circ p \circ \phi}_{C^k(\phi^{-1}(K))},
    \]
    can be made arbitrarily small (this is the norm we are interested in just slightly rewritten). To apply the Lemma the assumption $(p \circ \phi)(\phi^{-1}(K)) = p(K)\subseteq \tilde{L}$ for some compact set $\tilde{L} \subseteq W$, which is fixed prior, is still missing. But as mentioned before we can choose $\tilde{L}$ as any fixed compact neighbourhood of $L$ and if we then choose $\delta$ sufficiently small we automatically only consider functions $p$ with $p(K) \subseteq \tilde{L}$. Hence we can apply the Lemma. The resulting $\delta$ now may of course depend on $\tilde{L}$ but not on $p$ or $q$, which is what we are interested in.
...
\begin{proof}[Proof of Proposition \ref{whitney_top:prop:homotopy_construction}]
    ...
    \begin{enumerate}[(i)]
        ...
        \item $f(D_i) \subseteq W_i$,
        \item $w_i^{-1}(W_i) \subseteq \R^p$ is convex.
    \end{enumerate}
    ...
    \]
    the neighbourhood of $f$ determined by this data. Moreover we shall assume that $\mathcal{U}$ is of the form 
    \[
    ...
    \end{dcases}  
    \]
    These homotopies are well defined and satisfy the following properties for all $0 \leq k \leq n$.
    \begin{enumerate}[1)]
        ...
        \item \label{whitney_top:eq:homotopy_property_3}$h_t^k(D_i) \subseteq W_i$ for all $t \in [0,1]$,
        ...
        \item \label{whitney_top:eq:homotopy_property_6} $h_t^k(K_\alpha) \subseteq V_\alpha$ for all $\alpha \in J$ and the coordinate expressions $h_t^{t,\alpha} := \psi_\alpha^{-1} \circ h_t^k \circ \phi_\alpha$ satisfy
        \[
    ...
    The base case $n=0$ is clear. So suppose we have already constructed $\delta_1,\ldots,\delta_{n-1}$. Then for any $g \in \mathcal{V}_{n-1}$, $h_t^n$ is well-defined since $h_1^{n-1}(D_n) \subseteq W_n$, $g(D_n) \subseteq W_n$ and $w_n^{-1}(W_n)$ is convex, such that 
    \[
    ...
    \begin{itemize}
        ...
        \item $\lambda = \alpha(t)\lambda_n \circ o_n$ for some fixed $t \in [0,1]$,
        \item $K = o_n^{-1}(D_n \cap D_i)$,
        \item $V = w_n^{-1}(W_n \cap W_i)$.
    \end{itemize}
    Then we can write 
    \[
    (w_n^{-1} \circ h_t^n \circ o_n)(x) = (1-\lambda(x))p(x) + \lambda(x) q(x)
    \]
    for $x \in K$ and if (\ref{whitney_top:eq:homotopy_prop_eq1}) is sufficiently small then in particular $\norm{p-q}_{C^k(K)}$ can be made arbitrarily small. To apply the Lemma we still need to choose the compact set $L \subseteq V$, such that $q$ always satisfies $q(K) \subseteq L$ no matter which precise $g$ we used to define $q$. We fix $L$ as some compact neighbourhood of $(w_n^{-1} \circ f \circ o_n)(K) \subseteq V$. Then we can choose $\delta_n$ so small that $q(K) \subseteq L$ for any $g \in \mathcal{V}_n$. Now Lemma \ref{whitney_top:lem:lemma_for_homotopy_prop} tells us that $((1-\lambda)p + \lambda q)(K) \subseteq V$ for (\ref{whitney_top:eq:homotopy_prop_eq1}) sufficiently small, which is equivalent to $h_t^n(D_n \cap D_i) \subseteq W_i$. Of course we can then also achieve this for finitely many $i \in \N$ if (\ref{whitney_top:eq:homotopy_prop_eq1}) and $\delta_n$ are sufficiently small. But by a local finiteness argument only finitely many $D_i$ intersect $D_n$ and since $h_t^{n-1}(D_i) \subseteq W_i$ already holds for all $i \in \N$ and $h_t^{n} = h_1^{n-1}$ outside $D_n$ we conclude that we can also achieve $h_t^{n}(D_i) \subseteq W_i$ for all $i \in \N$.  

    For \ref{whitney_top:eq:homotopy_property_6}) we apply Lemma \ref{whitney_top:lem:lemma_for_homotopy_prop} to the same $p,q, \lambda$ and 
    \begin{itemize}
    ...
    \end{itemize}
    with the obvious domains and codomains. An anlogous argument as above shows that $h_t^{n}(K_\alpha) \subseteq V_\alpha$ holds for all $\alpha \in J$ if (\ref{whitney_top:eq:homotopy_prop_eq1}) and $\delta_n$ are sufficiently small. Furthermore we can write 
    \[
    h_t^{n,\alpha}(x) = \psi((1-\lambda(x)) p (\phi(x)) + \lambda(x) q(\phi(x)))
    \]
    ...
    \begin{itemize}
        ...
        \item $q = w_n^{-1} \circ g \circ o_n$,
        \item $\lambda = \lambda_{n-1} \circ o_n$,
        \item $K = o_n^{-1}(D_n \cap D_{n-1})$,
        \item $V = w_n^{-1}(W_n \cap W_{n-1})$,
        \item $\psi = w_{n-1}^{-1} \circ w_n$.
    \end{itemize}
    Again if $\delta_n$ is sufficiently small we obtain $q(K) \subseteq L$ for any $g \in \mathcal{V}_n$, where $L \subseteq V$ is some fixed compact neighbourhood of $(w_n^{-1} \circ f \circ o_n)(K) \subseteq V$. Write 
    \[
