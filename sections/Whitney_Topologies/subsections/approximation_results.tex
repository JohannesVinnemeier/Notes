...
In the following we use the term ``manifold with boundary'' to refer to a smooth manifold, which may have empty or non-empty boundary. So let $M$ and $N$ be manifolds with boundary. Consider the set $C^\infty(M,N)$ of smooth maps from $M$ to $N$. On this set one can define the so called Whitney topologies, which provide a precise language to make sense of such statements as "slighlty perturbing $f$ yields a map transversal to a given submanifold". In the following we will proof a number of useful approximation results of this kind concerning immersions, one-to-one immersions, embeddings and transversal maps.
