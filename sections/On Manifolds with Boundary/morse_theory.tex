\subsection{Morse Theory}
    ...
    \begin{remark}
        We may also formulate Theorem \ref{thm:topology_when_crossing_critical_point} and \ref{thm:cell_decomp} for handle bodies instead of CW-complexes. The proof is essentially the same but instead of construction a deformation retract $F^{-1}(-\infty,c-\epsilon] \to M_{c-\epsilon} \cup_\phi D^\lambda$ one constructs a homeomorphism $F^{-1}(-\infty,c-\epsilon] \to M_{c-\epsilon} \cup_\phi D^\lambda \times D^{n-\lambda}$.
    \end{remark}
