\subsection{Regular values and transversality}
    ...
    \begin{proof}
        ...
        \]
        where $\mathrm{D}(N)$ denotes the double of $N$ and define $F := \iota \circ f$. Then $Y := \iota(X)$ is a submanifo  ld of $\mathrm{D}(N)$ and $X^\prime = F^{-1}(Y)$. Furthermore, since $\iota$ is an embedding (and $\mathrm{dim}(N) = \mathrm{dim}(\mathrm{D}(N))$) it follows that 
        \[
    ...
    \begin{proof}
        ...
        and 
        \[
        g \colon M \to \mathrm{int}(N), \; p \mapsto f(p).
        \]
        Using the Transversality Homotopy Theorem for the case $\partial N = \emptyset$ (see \cite{guillemin2010differential} p. 70) we get that $g$ is smoothly homotopic to a map $g^\prime \colon M \to \mathrm{int}(N)$, such that 
        \[
            g^\prime \pitchfork Y \text{ and } \partial g^\prime \pitchfork Y.
        \]
        Now define $f^\prime := \iota \circ g$, where $\iota \colon \mathrm{int}(N) \hookrightarrow N$ is the inclusion. Since $f^\prime$ does not intersect $X$ at boundary points, i.e. $\mathrm{im}(f^\prime) \cap X \subseteq \mathrm{int}(N)$, the transversality properties of $g^\prime$ directly imply
        \[
            f^\prime \pitchfork X \text{ and } \partial f^\prime \pitchfork X.
        \]
