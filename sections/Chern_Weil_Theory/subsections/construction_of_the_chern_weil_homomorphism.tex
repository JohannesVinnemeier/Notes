...
\begin{lemma}
    Let $\Sigma^r(V^\ast)$ be the space of $r$-linear symmetric maps $V \times \ldots \times V \to \R$. Then 
\begin{align*}
...
Under the isomorphism $\Phi_r$ the invariant polynomials correspond to the invariant $r$-linear symmetric maps, where an $r$-linear symmetric map $\overline{\eta} \colon \R^{k \times k} \times \ldots \times \R^{k \times k} \to \R$ is called invariant if 
\[
...
\]
So this definition agrees with the one in \cite{emilia_globale_ana}, but has the benefit of being coordinate independent. We will mostly work with this formula. Note that $\Omega_i$ can always be written as a sum of differential forms of the required form, where the $A_i$ are matrices with only one non-zero entry being equal to $1$. Alternatively one can also think of $\overline{\eta}(\Omega_1,\ldots,\Omega_r)$ as the $l$-form obtained by evaluating the polynomial $\eta = \Phi_r(\overline{\eta})$ on the differential form valued entries of the matrices $\Omega_i$, where multiplication is given by the wedge product. 
