...
\begin{lemma}
    ...
    Let $\overline{\eta} \in I_r(k)$ and $X_1,\ldots,X_r,Y \in \K^{k \times k}$. Then 
    \[
...
\begin{proof}
    Let $A(t) \in \mathrm{GL}(k,\K)$ be a smooth curve with $A(0) = \mathrm{id}$ and $\dot{A}(0) = Y$. Then 
    \begin{align*}
...
Before stating the second Lemma let us define the commutator of two differential forms $\Omega_1 \in \Omega^{l_1}(M,\K^{k \times k})$ and $\Omega_2 \in \Omega^{l_2}(M,\K^{k \times k})$ by 
\[
[\Omega_1,\Omega_2] := \Omega_1 \wedge \Omega_2 - (-1)^{l_1l_2} \, \Omega_2 \wedge \Omega_1.
\]
If $\Omega_i = \omega_i \otimes A_i$ for some $\omega_i \in \Omega^{l_i}(M,\K)$ and $A_i \in \Omega^0(M,\K^{k \times k})$ then this can also be written as 
\[
...
\begin{proof}[Proof of Theorem \ref{chern_weil:thm:chern_weil_thm}]
    ...
    \end{equation}
    for matrix valued $2$-forms $\Omega_i \in \Omega^2(M,\K^{k \times k})$. By linearity we may assume that all except for one entry in the matrix $\Omega_i = (\Omega_i^{jl})_{j,l=1}^k$ of $2$-forms vanish, so in particular
    \[
    ...
    \]
    In order to do this choose a local frame $e_1,\ldots,e_k$ of $E$ and write $s = s^i \pi^\ast e_i$ for smooth functions $s^i \colon M \times \R \to \K$. Then $\pi^\ast\iota_0^\ast s = (s^i \circ \iota_0 \circ \pi) \pi^\ast e_i$ and thus 
    \begin{align*}
