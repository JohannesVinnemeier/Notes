...
\begin{proof}
    ...
    \end{tikzcd}\]
    commutes. So $P \times_G F$ is locally trivial. Next we have to compute the transition maps. Let $U_\alpha, U_\beta \subseteq B$ be two trivialising neighbourhoods for $P$ with  local trivialisations $\lambda_\alpha, \lambda_\beta$ and transition map $\tau_{\alpha,\beta} \colon U_\alpha \cap U_\beta \to G$. Moreover let $\kappa_\alpha, \kappa_\beta$ be the corresponding local trivialisations for $P \times_G F$ as constructed above. Then we compute 
    \begin{align*}
...
Given a group homomorphism $\phi \colon G \to H$ of topological groups we can view $H$ as a left $G$-space by $g \cdot h := \phi(g)h$. Of course this action may not be effective anymore but the Borel construction can still be used to construct a $H$-principal bundle out of a $G$-principal bundles using this action:
...
\begin{proposition}
    ...
    \]
    together with the map $\pi \colon P \times_\phi H \to B$ given by $[(y,h)] \mapsto p(y)$ is a $H$-principal bundle. Moreover this construction is natural in the sense that if $f \colon B^\prime \to B$ is a continuous map then 
    \[
