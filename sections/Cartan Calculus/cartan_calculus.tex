...
\]
Of course it is not clear a priori that this definition does not depend on the choice of $X^\prime$ and $\alpha^\prime$. However, the following proposition, which is one of the formulas of Theorem \ref{thm:cartan_calculus}, shows that it is in fact independent.
...
\begin{proof}
    Assume that $\alpha_t$ is defined for all $t \in \R$ and consider the smooth two-parameter family $\phi_t^\ast \alpha_s$ for $(t,s) \in [0,1] \times \R$. Then for each $p \in M$ we have a smooth map
    \[
        g \colon [0,1] \times \R \to T_p^\ast M, \: (t,s) \mapsto (\phi_t^\ast \alpha_s)_p
    \]
    and
    \[
        \frac{d}{dt}\bigg|_{t=t_0} (\varphi_t^\ast \alpha_t)_p = \frac{\partial g}{\partial t}(t_0,t_0) + \frac{\partial g}{\partial s}(t_0,t_0) 
    \]
    By Proposition \ref{prop:lie_derivative_formula} we have:
    \[
    \frac{\partial}{\partial t}\bigg|_{t=t_0} \varphi_t^\ast \alpha_s = \varphi_{t_0}^\ast(\mathcal{L}_{X_{t_0}} \alpha_s)
    \]
    Moreover, by writing $\alpha_s$ in local coordinates it is easily seen that
    \[
...
\subsection{Further computation rules}
...
\begin{proposition}
    ...
    Let $M$ be a manifold without boundary, $X_t$ a time dependent vector field on $M$ and $\alpha \in \Omega^r(M).$ Denote by $\phi_t$ the (time-dependent) flow of $X_t$ and consider the open set $M_{t_0} = \{p\in M \mid \phi_t(p) \text{ exists for } t=t_0\}$. Then
    \[
...
\end{proposition}
\begin{proof}
    If $X_t$ does not depend on time, i.e. $X_t = X_0$ for all $t$ where it is defined, the proposition follows from a simple computation:
    \begin{align*}
        \frac{d}{dt}\bigg|_{t=t_0}(\phi_t^\ast\alpha)_p &= \frac{d}{dt}\bigg|_{t=0}(\phi_{t+t_0}^\ast\alpha)_p = \frac{d}{dt}\bigg|_{t=0}(\phi_{t_0}^\ast\phi_t^\ast\alpha)_p \\
        &= \phi_{t_0}^\ast \frac{d}{dt}\bigg|_{t=0} (\phi_t^\ast\alpha)_p = \phi_{t_0}^\ast (\mathcal{L}_{X_0} \alpha)_p = \phi_{t_0}^\ast (\mathcal{L}_{X_{t_0}} \alpha)_p
    \end{align*}
    In the general case we argue as follows: The maps
    \[
    \Omega^r(M_{t_0}) \ni \alpha \mapsto \frac{d}{dt}\bigg|_{t=t_0}\phi_t^\ast\alpha
    \]
    and 
    \[
        \Omega^r(M_{t_0}) \ni \alpha \mapsto \phi_{t_0}^\ast \mathcal{L}_{X_{t_0}} \alpha 
    \]
    are both differential operators satisfying the same Leibniz rule and commute with $d$. For the second one this just follows from Proposition \ref{prop:lie_derivative_properties} and for the first one we can carry out precisely the same proof as of Proposition \ref{prop:lie_derivative_properties}. Thus we can again establish equality by showing that they agree on $\Omega^0(M_{t_0})$. So for $f \in \Omega^0(M_{t_0})$ we compute:
    \begin{align*}
        \frac{d}{d t}\bigg|_{t=t_0} (\phi_t^\ast f)_p &= \frac{d}{d t}\bigg|_{t=t_0} f(\phi_t(p)) \\
        &= d_{\phi_{t_0}(p)}f((X_{t_0})_{\phi_{t_0}(p)}) \\
        &= (X_{t_0} \cdot f)(\phi_{t_0}(p))\\
        &= \phi_{t_0}^\ast(X_{t_0} \cdot f)(p)\\
        &= \phi_{t_0}^\ast(\mathcal{L}_{X_{t_0}}f)_p
    \end{align*}
    This shows the statement.
\end{proof}

Here are two more rules for computing the time-derivative of a smooth family of $r-$forms, which are occasioanlly useful:

\begin{lemma}
    \label{lem:cartan_calculus_further_rules}
    Let $\alpha_t$ be a smooth time-dependent family of differential forms on a manifold $M$ and $f \colon N \to M$ a smooth map. Then the following holds:
    \begin{enumerate}[(i)]
        \item $\frac{d}{d t}f^*\alpha_t = f^*\frac{d}{d t} \alpha_t$
        \item $\frac{d}{d t} d\alpha_t = d(\frac{d}{d t} \alpha_t)$
    \end{enumerate} 
\end{lemma}
\begin{proof}
    Both statements can easily be verified by directly computing both sides of the equation in local coordinates. For $(ii)$ we have in fact already seen this computation in the proof of Lemma \ref{prop:lie_derivative_properties}.
\end{proof}
...
\begin{example}
    Let $X_t$ be some time dependent vector field on a manifold $M$ with (time-dependent) flow $\phi_t$. Furthermore, let $\alpha_t$ be a smooth family of differential forms and $f_t$ a smooth family of functions on $M$. We want to compute:
    \[
