...
\begin{proof}
    ...
    \]
    To see this, we can explicitly express both sides in local coordinates and then use (\ref{eq:cartan_calc_further_rule_2}) and (\ref{eq:cartan_calc_further_rule_3}) from Lemma \ref{lem:cartan_calculus_further_rules} to reduce this to the case, where $\alpha$ is a $0$-form, which follows immediately. Essentially, this is again using the trick that two differential operators are equal if they satisfy the same Leibniz rule, commute with $d$ and agree on $0$-forms. 
\end{proof}

Here are some more rules for computing the time-derivative of a smooth family of $r-$forms, which are occasioanlly useful:

\begin{lemma}
    \label{lem:cartan_calculus_further_rules}
    Let $\alpha_t, \beta_t$ be smooth time-dependent families of differential forms on a manifold $M$ and $f \colon N \to M$ a smooth map. Then the following holds:
    \begin{enumerate}[(i)]
        \item $\frac{d}{d t}f^*\alpha_t = f^*\frac{d}{d t} \alpha_t$ \label{eq:cartan_calc_further_rule_1}
        \item $\frac{d}{d t} d\alpha_t = d(\frac{d}{d t} \alpha_t)$\label{eq:cartan_calc_further_rule_2}
        \item $\frac{d}{dt}(\alpha_t\wedge \beta_t) = \frac{d}{dt}\alpha_t \wedge \beta_t + \alpha_t \wedge \frac{d}{dt}\beta_t$ \label{eq:cartan_calc_further_rule_3}
    \end{enumerate} 
\end{lemma}
\begin{proof}
    All statements can easily be verified by directly computing both sides of the equation in local coordinates. For (\ref{eq:cartan_calc_further_rule_2}) and (\ref{eq:cartan_calc_further_rule_3}) we have in fact already seen this computation in the proof of Lemma \ref{prop:lie_derivative_properties}.
\end{proof}
