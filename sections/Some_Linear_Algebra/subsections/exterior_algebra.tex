...
\]
for $\sigma \in S(k) \footnote{S(k) \text{ denotes the symmetric group on $k$ elements}}$ given by 
\[
...
\begin{remark}
    Using these universal properties it is easy to see that the assignments $V \mapsto \Lambda^k V$ and $V \mapsto \Lambda^\ast V$ are functorial.
\end{remark}
