...
(\alpha,\beta) &\mapsto \alpha \wedge \beta := \mathrm{Alt}(\alpha \otimes \beta).
\end{align*}
...
\]
is a basis of $\Lambda^k V$. The exterior algebra is then defined as 
\[
...
We can also characterize the exterior power and the exterior algebra by means of a universal property. To this end let 
\[
...
\begin{definition}[Universal property of the exterior power]
    ...
    The ($k$-fold) exterior power of $V$, denoted $\Lambda^k V$, is a vector space together with a linear map 
    \[
    \phi \colon V^{\otimes k} \to \Lambda^k V,
    \]
    which vanishes on $S^k(V)$, such that for any linear map 
    \[
    T \colon V^{\otimes k} \to W
    \]
    into a vector space $W$ with $T|_{S^k(V)} = 0$ there exists a unique linear map 
    \[
...
\begin{definition}[Universal property of the exterior algebra]
    ...
    \[
    \psi \colon V \to \Lambda^\ast V,
    \]
    which satisfies $\psi(v)^2 = 0$ for all $v\in V$ and such that for any linear map 
    \[
    \psi^\prime \colon V \to A^\prime
    \]
    into an algebra $A^\prime$ with $\psi^\prime(v)^2 = 0$ for all $v\in V$ there exists a unique algebra homomorphism
    \[
    L \colon \Lambda^k V \to A^\prime
    \]
    making the following diagram commute 
    \[\begin{tikzcd}[column sep = 3em]
	{V} & \Lambda^\ast V\\
	...
	\arrow["\psi^\prime"', from=1-1, to=2-2]
...
\begin{remark}
    In Definition \ref{linalg:def:universal_property_exterior_power} we can think of $T$ descending to a map $\widetilde{T}$, whereas in Definition \ref{linalg:def:universal_property_exterior_algebra} we can think of $L$ extending the map $\psi^\prime$.
\end{remark}

\begin{lemma}
    \label{linalg:lem:universal_property_exterior_power}
    The construction of $\Lambda^k V \subseteq V^{\otimes k}$ at the beginning of the section together with the antisymmetrization map $\mathrm{Alt}$ satisifes the universal property in Definition \ref{linalg:def:universal_property_exterior_power}.
\end{lemma}

\begin{proof}
    First of all it follows easily from the definition, that $\mathrm{Alt}$ vanishes on $S^k(V)$. Uniqueness is also clear. To show existence choose a basis $e_1,\ldots,e_n \in V$ of $V$. As mentioned above this yields a basis of $\Lambda^kV$ and we define $\widetilde{T}$ on this basis (necessarily) by 
    \[
    \widetilde{T}(e_{i_1} \wedge \ldots \wedge e_{i_k}) := T(e_{i_1} \otimes \ldots \otimes e_{i_k})
    \]
	...
	\arrow["\widetilde{T}"', dashed, end anchor = {[xshift = 5, yshift = -3]}, from=2-1, to=1-2]
\end{tikzcd}\]
does in fact commute it suffices to verify commutativity on basis vectors $e_{i_1} \otimes \ldots \otimes e_{i_k}$ of $V^{\otimes k}$ (where the tuple $(i_1,\ldots,i_k)$ is not necessarily ordered!). But from $T|_{S^k(V)} = 0$ we can deduce that 
\[
T(v_1 \otimes \ldots \otimes v_i \otimes \ldots \otimes v_j \otimes \ldots \otimes v_k) = -T(v_1 \otimes \ldots \otimes v_j \otimes \ldots \otimes v_i \otimes \ldots \otimes v_k)
\]
for any $v_1,\ldots,v_k \in V$ and $1 \leq i,j \leq k$ with $i\neq j$. So after a suitable permutation of $(i_1,\ldots,i_k)$ we can assume this tuple to be ordered, such that the statement follows from the definiton of $\widetilde{T}$.
\end{proof}
...
\begin{proof}
    Again uniqueness is clear. For the existence of $L$ we define it on $\Lambda^k V$ (necessarily) by 
    \[
...
To conclude this section let us mention another commonly used construction of the exterior power and exterior algebra. Firstly note that the quotient $\modulo{V^{\otimes k}}{S^k(V)}$ together with the quotient map 
\[
V^{\otimes k} \to \modulo{V^{\otimes k}}{S^k(V)}
\]
also clearly satisifies the universal property in Definition \ref{linalg:def:universal_property_exterior_power}. Hence this is also a valid and commonly used construction of the exterior power. In particular this implies that there exists a unique isomorphism 
\[
...
\]
Similiarly we can also define the exterior algebra as a quotient of the tensor algebra
\[
