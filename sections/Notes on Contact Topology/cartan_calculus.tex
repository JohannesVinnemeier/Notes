...
Let $M$ be a manifold (with or without boundary). Given a smooth family $\alpha_t$ of differential forms on $M$ indexed over $t \in [0,1]$ we want to find a family of diffeomorphisms $\varphi_t \colon M \to M$ (i.e. an isotopy), such that $\varphi_0 = id_M$ and 
\[
...
\textcolor{red}{\textbf{For simplicity let $\partial M = \emptyset$ for now. Still need to resolve that!!!}}
...
\begin{proof}
    ...
    \]
    The same proof as of Proposition \ref{prop:lie_derivative_properties} shows that 
    \[
    \beta \mapsto \frac{\partial}{\partial t}\bigg|_{t=t_0} \varphi_t^\ast \beta
    \]
    is a differential operator commuting with $d$ and satisfying the same Leibniz rule as 
    \[
    \beta \mapsto \varphi_{t_0}^\ast(\mathcal{L}_{X_{t_0}}\beta).
    \]
    Thus we only need to show equality on $\Omega^0(M)$. So for $f \in \Omega^0(M)$ we compute:
    \begin{align*}
        \frac{\partial}{\partial t}\bigg|_{t=t_0} (\varphi_t^\ast f)_p &= \frac{\partial}{\partial t}\bigg|_{t=t_0} f(\varphi_t(p)) \\
        &= d_{\varphi_{t_0}(p)}f((X_{t_0})_{\varphi_{t_0}(p)}) \\
        &= (X_{t_0} \cdot f)(\varphi_{t_0}(p))\\
        &= \varphi_{t_0}(p)^\ast(X_{t_0} \cdot f)(p)\\
        &= \varphi_{t_0}^\ast(\mathcal{L}_{X_{t_0}}f)_p
    \end{align*}
    This shows the claim. Moreover, by writing $\alpha_s$ in local coordinates it is easily seen that
    \[
